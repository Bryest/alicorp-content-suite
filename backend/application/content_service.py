...
from ..domain.brand import BrandRepository, ForbiddenWords
...
class ContentService:
    ...
    async def _extract_forbidden_words(self, scored, manual) -> list[str]:
        """Read forbidden words directly from the structured domain field.

        We deliberately do NOT parse them out of the FORBIDDEN chunk text:
        that text is descriptive prose generated by the LLM and is unsafe
        to depend on. The list of words is the single source of truth.
        """
        return list(manual.forbidden_words or [])
